...
func main() {
	...
	s, err := NodeSSHHosts(client)
	if err != nil {
		panic(err)
	}
	fmt.Println(s)

	signer, err := makePrivateKeySignerFromFile("/Users/dp/.ssh/id_rsa")
	if err != nil {
		panic(err)
	}

	runSSHCommand("ls -l", "root", "39.101.222.65:22", signer)
}
...
// case.
func NodeSSHHosts(c kubernetes.Interface) ([]string, error) {
...
func waitListSchedulableNodesOrDie(c kubernetes.Interface) *v1.NodeList {
